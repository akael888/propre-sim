"use client";
import { updateSlideData } from "@/app/lib/action";
import { User } from "next-auth";
import Link from "next/link";

export default function SaveAreaClient({
  slideID,
  user,
  submitAction,
  textAreaData,
  ...
  submitAction?: ((formData: FormData) => void) | string;
  textAreaData: string;
  slideData: {
    title: string;
    description: string;
  ...
  return (
    <>
      <div className="flex flex-col gap-5 max-w-screen w-full p-1 ">
        {user ? (
          <form
            action={slideID ? updateSlideData : submitAction}
